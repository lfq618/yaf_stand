...
class Yaf_Route_Regex implements Yaf_Route_Interface {
	...
	public function __construct($match, $route, $map, $verify = null) {
		...
		{
			throw new Yaf_Exception_RouterFailed('Expects a string as the first parameter');
		}
		if (!is_array($route))
		{
			throw new Yaf_Exception_RouterFailed('Expects a array as the second parameter');
		}
		if (!is_array($map))
		{
			throw new Yaf_Exception_RouterFailed('Expects a array as the third parameter');
		}
		if ($verify != null && !is_array($verify))
		{
			throw new Yaf_Exception_RouterFailed('Expects an array as verify parameter');
		}
		$this->_route = $match;
