...
class Yaf_Loader {
	...
	public function autoload($class) {
		...
		{
			throw new Yaf_Exception('You should not use ' . self::YAF_LOADER_RESERVERD . ' as class name prefix');
		}
		...
		if ($directory != '' && $appDir == '')
		{
			Yaf_Exception::trigger_error('Couldn\'t load a framework MVC class without an ' . 'Yaf_Application initializing ', E_USER_WARNING);
		}
		...
		if ($this->internal_autoload($class, $directory))
		{
			...
			)
			{
				...
				{
					Yaf_Exception::trigger_error('Could not find class ' . $className . ' in ' . $directory, E_USER_ERROR);
				}
				else
				{
					return false;
		...
		else
		{
			if ($splAutoLoadIni == false)
			{
				Yaf_Exception::trigger_error('Could not find script ' . ($directory != '' ? $directory : $this->resolveDirectory($class)) . DIRECTORY_SEPARATOR . $this->resolveClass($class), E_USER_WARNING);
			}
			else
			{
				return false;
	...
	public function registerLocalNamespace($namespace) {
		...
		{
			throw new Yaf_Exception('Invalid parameters provided, must be a string, or an array');
		}
	...
	private function _securityCheck($filename) {
		...
		{
			throw new Yaf_Exception('Security check: Illegal character in filename');
		}
	}
