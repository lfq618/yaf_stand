...
class Yaf_Request_Http extends Yaf_Request_Abstract {
	...
	public function __set($name, $value) {
		throw new Yaf_Request_Exception('Use setParam to set a parameter of request');
	}
	...
	public function setRequestUri($requestUri = null) {
		if ($requestUri === null)
		{
			...
			elseif ( // IIS7 with URL Rewrite: make sure we
		...
		if ($baseUri === null)
		{
			...
			else
			{
				...
				do
				{
					...
					$baseUri = '/' . $seg . $baseUri;
					++$index;
				} while (($last > $index) && (false !== ($pos = strpos($path, $baseUri))) && (0 != $pos));
			}
			...
			if ((strlen($requestUri) >= strlen($baseUri)) && ((false !== ($pos = strpos($requestUri, $baseUri))) && ($pos !== 0))
			)
			{
